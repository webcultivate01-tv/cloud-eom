...
export default function ForgotPassword() {
  ...
  const inputCls = "w-full px-4 py-3 border border-gray-200 rounded-xl text-sm bg-gray-50 outline-none focus:border-brand-700 transition-colors box-border font-[inherit]";
    ...
    <div className="min-h-[calc(100vh-120px)] bg-gray-50 flex items-center justify-center px-4 py-8">
      <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-8 md:p-10 w-full max-w-md">
        ...
        <div className="flex items-center gap-2.5 mb-6">
          ...
          <div>
            <p className="text-brand-700 font-black text-sm tracking-widest m-0">CLOUD GRAPHICS</p>
            ...
            <div className="flex items-center mb-7">
              {STEPS.map((label, i) => (
                <div key={i} className="flex items-center gap-1.5">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center font-bold text-xs shrink-0 ${step >= i + 1 ? "bg-brand-700 text-white" : "bg-gray-200 text-gray-400"}`}>
                    {step > i + 1 ? "✓" : i + 1}
                  </div>
                  <span className={`text-xs whitespace-nowrap hidden sm:inline ${step >= i + 1 ? "text-brand-700 font-bold" : "text-gray-300"}`}>{label}</span>
                  {i < STEPS.length - 1 && <div className={`w-6 md:w-8 h-0.5 mx-1 ${step > i + 1 ? "bg-brand-700" : "bg-gray-200"}`} />}
                </div>
              ))}
            </div>
          </>
        ...
        {step === 1 && (
          <form onSubmit={handleSendOTP} className="flex flex-col gap-4">
            <div className="bg-brand-50 border border-brand-200 rounded-xl px-4 py-3 text-brand-700 text-sm leading-relaxed">
            ...
            <button type="submit" disabled={loading} className="w-full py-3.5 bg-brand-700 hover:bg-brand-800 text-white rounded-xl font-bold text-sm border-none cursor-pointer transition-colors disabled:opacity-60">
              {loading ? "Sending OTP…" : "Send OTP →"}
            </button>
            <Link to="/login" className="block text-center text-brand-700 font-semibold text-sm hover:underline">← Back to Login</Link>
          </form>
        )}
        ...
        {step === 2 && (
          <form onSubmit={handleVerifyOTP} className="flex flex-col gap-4">
              ...
              <div className="flex gap-2.5 justify-center">
                {[0,1,2,3,4,5].map((i) => (
                  <input key={i} id={`fp-otp-${i}`} type="text" inputMode="numeric" maxLength={1}
                    className="w-11 text-center text-2xl font-black border-2 border-gray-200 rounded-xl outline-none focus:border-brand-700 text-gray-900 box-border"
                    style={{ height: "52px" }}
                    value={otp[i] || ""}
                    onChange={(e) => {
            ...
            <button type="submit" disabled={otp.length < 6} className="w-full py-3.5 bg-brand-700 hover:bg-brand-800 text-white rounded-xl font-bold text-sm border-none cursor-pointer transition-colors disabled:opacity-50">
              Verify OTP →
            </button>
            <div className="flex items-center justify-center gap-2">
              <span className="text-gray-400 text-xs">Didn't receive it?</span>
              <button type="button" onClick={handleResendOTP} disabled={loading} className="bg-transparent border-none text-brand-700 font-bold text-xs cursor-pointer hover:underline">
                {loading ? "Sending…" : "Resend OTP"}
              </button>
            </div>
            <button type="button" onClick={() => setStep(1)} className="block text-center text-brand-700 font-semibold text-sm bg-transparent border-none cursor-pointer hover:underline">← Change Email</button>
          </form>
        )}
        ...
        {step === 4 && (
          <div className="text-center flex flex-col items-center gap-3 py-4">
            ...
            <button onClick={() => navigate("/login")} className="w-full mt-2 py-3.5 bg-brand-700 hover:bg-brand-800 text-white rounded-xl font-bold text-sm border-none cursor-pointer transition-colors">
              Go to Login →
            </button>
          </div>
        )}
