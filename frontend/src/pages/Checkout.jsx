...
export default function Checkout() {
  ...
  const [shipping, setShipping] = useState({
    fullName: "", phone: "", address: "", addressLine2: "",
    ...
    const options = {
      ...
      config: {
        display: {
          ...
          blocks: { upi_block: { name: "Pay via UPI", instruments: [{ method: "upi" }] } },
      ...
      <div style={s.layout}>
            ...
            <div style={s.card}>
                ...
                <div style={{ display: "flex", gap: "10px" }}>
                  {ADDR_TYPES.map((t) => (
                    <button
                      ...
                      style={{
                        ...s.addrTypeBtn,
                        ...(shipping.addressType === t ? s.addrTypeBtnActive : {}),
                      }}
                      onClick={() => setShipping({ ...shipping, addressType: t })}
                    >
                      {t === "Home" ? "🏠" : t === "Work" ? "🏢" : "📍"} {t}
                    </button>
                  ))}
              ...
              <div style={s.formGrid}>
                ...
                <div style={s.formGroup}>
                  ...
                  <input style={s.input} placeholder="10-digit mobile number" value={shipping.phone} onChange={(e) => setShipping({ ...shipping, phone: e.target.value })} maxLength={10} />
                </div>
              </div>

              <div style={s.formGroup}>
                <label style={s.label}>Address Line 1 * <span style={{ color: "#999", fontWeight: "400" }}>(House no, building, street)</span></label>
                <input style={s.input} placeholder="e.g. 12, Shivaji Nagar, MG Road" value={shipping.address} onChange={(e) => setShipping({ ...shipping, address: e.target.value })} />
              </div>

              <div style={s.formGroup}>
                <label style={s.label}>Address Line 2 <span style={{ color: "#999", fontWeight: "400" }}>(optional)</span></label>
                <input style={s.input} placeholder="e.g. Opposite Bus Stand" value={shipping.addressLine2} onChange={(e) => setShipping({ ...shipping, addressLine2: e.target.value })} />
              </div>

              <div style={s.formGroup}>
                <label style={s.label}>Landmark <span style={{ color: "#999", fontWeight: "400" }}>(optional)</span></label>
                <input style={s.input} placeholder="e.g. Near City Mall" value={shipping.landmark} onChange={(e) => setShipping({ ...shipping, landmark: e.target.value })} />
              </div>
              ...
              <div style={s.formGrid}>
                ...
                <div style={s.formGroup}>
                  ...
                  <input style={s.input} placeholder="6-digit pincode" value={shipping.pincode} onChange={(e) => setShipping({ ...shipping, pincode: e.target.value })} maxLength={6} />
                </div>
              </div>

              <div style={s.formGroup}>
                <label style={s.label}>State *</label>
                <select style={s.input} value={shipping.state} onChange={(e) => setShipping({ ...shipping, state: e.target.value })}>
              ...
              <div style={s.formGroup}>
                <label style={s.label}>Order Note <span style={{ color: "#999", fontWeight: "400" }}>(optional)</span></label>
                <textarea style={{ ...s.input, height: "60px", resize: "vertical" }} placeholder="Special instructions for delivery..." value={note} onChange={(e) => setNote(e.target.value)} />
              </div>

              <button type="button" style={s.nextBtn} onClick={() => { if (validateShipping()) setStep(2); }}>
          ...
          {step === 3 && (
            <div style={s.card}>
              ...
              <div style={s.reviewSection}>
                <h3 style={s.reviewLabel}>Delivering To</h3>
                <div style={s.reviewAddrBox}>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
                    <span style={s.reviewAddrType}>
                      {shipping.addressType === "Home" ? "🏠" : shipping.addressType === "Work" ? "🏢" : "📍"} {shipping.addressType}
                    </span>
                  </div>
                  <p style={{ fontWeight: "700", color: "#1a1a1a", margin: "0 0 2px" }}>{shipping.fullName}</p>
                  <p style={{ color: "#666", fontSize: "0.85rem", margin: "0 0 2px" }}>📞 {shipping.phone}</p>
                  <p style={{ color: "#555", fontSize: "0.85rem", margin: 0, lineHeight: 1.6 }}>
                    {shipping.address}{shipping.addressLine2 ? ", " + shipping.addressLine2 : ""}
                    {shipping.landmark ? `, Near: ${shipping.landmark}` : ""}<br />
              ...
              {note && (
                <div style={s.reviewSection}>
                  <h3 style={s.reviewLabel}>Order Note</h3>
                  <p style={{ color: "#555", fontSize: "0.85rem" }}>{note}</p>
                </div>
              )}
            ...
            <div style={s.card}>
              ...
              {paymentMethod === "razorpay" && (
                <div style={s.razorpayBox}>
                  ...
                  <div style={s.upiRow}>
                    <input style={s.upiInput} placeholder="yourname@upi  (e.g. 9876543210@paytm)" value={upiId} onChange={(e) => setUpiId(e.target.value)} />
                    {upiId.trim() && <button type="button" style={s.upiClearBtn} onClick={() => setUpiId("")}>✕</button>}
                  </div>
                  <p style={s.razorpayNote}>🔒 Powered by Razorpay — UPI · Cards · Net Banking · Wallets. 100% secure &amp; encrypted.</p>
                </div>
              )}
              {paymentMethod === "cod" && (
                <div style={s.codNote}>📦 Your order will be processed and delivered to you. Pay in cash at the time of delivery.</div>
              )}
              ...
              <div style={s.navRow}>
                ...
                <button type="button" style={{ ...s.placeBtn, opacity: loading ? 0.7 : 1 }} disabled={loading} onClick={handlePayNow}>
        ...
        <div style={s.summary}>
          ...
          <div style={{ ...s.summaryRow, marginTop: "8px" }}>
            <span style={{ color: "#999", fontSize: "0.75rem" }}>Shipping</span>
            <span style={{ color: "#2e7d32", fontWeight: "700", fontSize: "0.8rem" }}>FREE</span>
          </div>
          ...
          {step >= 2 && shipping.city && (
            <div style={s.summaryAddrSnip}>
              <p style={{ color: "#888", fontSize: "0.72rem", margin: "0 0 2px", textTransform: "uppercase", letterSpacing: "0.5px" }}>Delivering to</p>
              <p style={{ color: "#333", fontSize: "0.78rem", fontWeight: "600", margin: 0 }}>{shipping.fullName}</p>
              <p style={{ color: "#666", fontSize: "0.75rem", margin: 0 }}>{shipping.city}{shipping.state ? ", " + shipping.state : ""} {shipping.pincode}</p>
            </div>
          )}
        </div>
      </div>
...
const s = {
  ...
  addrTypeBtn: { padding: "8px 16px", border: "1.5px solid #e0e0e0", borderRadius: "6px", background: "#fafafa", cursor: "pointer", fontSize: "0.82rem", fontWeight: "600", color: "#555", transition: "all 0.15s" },
  addrTypeBtnActive: { borderColor: "#c41230", background: "#fff5f6", color: "#c41230" },
  ...
  reviewAddrBox: { background: "#f7f7f7", borderRadius: "8px", padding: "14px 16px" },
  reviewAddrType: { background: "#e0e0e0", padding: "2px 10px", borderRadius: "4px", fontSize: "0.75rem", fontWeight: "700", color: "#555" },
  ...
  summaryAddrSnip: { marginTop: "12px", paddingTop: "12px", borderTop: "1px solid #e0e0e0" },
