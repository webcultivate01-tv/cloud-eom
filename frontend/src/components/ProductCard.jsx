...
export default function ProductCard({ product, badge }) {
  ...
  return (
    ...
    >
      {/* ── Image ── */}
      <div className="relative overflow-hidden aspect-[4/5] bg-[#f1f5f8]">
        <img
          ref={imgRef}
      ...
      <div className="flex flex-col flex-1 px-4 pt-4 pb-4 md:px-5 md:pt-5 md:pb-5">
        <p className="text-slate-400 text-[10px] font-semibold uppercase tracking-[0.16em] mb-2">
          {product.category}
        </p>
        <h3 className="font-display text-slate-900 text-[15px] md:text-[17px] font-bold leading-snug tracking-[-0.01em] line-clamp-2 mb-4">
          {product.name}
        </h3>

        <div className="mt-auto flex items-center justify-between gap-2">
          <span className="font-display text-brand-700 text-xl md:text-[26px] font-bold leading-none tracking-tight">
          ...
          {soldOut ? (
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 border border-slate-900 text-slate-900 bg-white text-[11px] md:text-xs font-semibold px-3.5 md:px-4 py-2 rounded-full whitespace-nowrap cursor-pointer transition-all duration-200 hover:bg-slate-900 hover:text-white"
            >
